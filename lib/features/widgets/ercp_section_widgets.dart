...
class ErcpSection extends StatelessWidget {
  final Patient patient;

  const ErcpSection({super.key, required this.patient});

  @override
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        context.read<ErcpCubit>().loadForPatient(patient.id!, force: true);
      } catch (_) {}
    });

    return BlocBuilder<ErcpCubit, ErcpState>(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: const [
                            ...
                            SizedBox(width: 8),
                            Text('Add ERCP', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                          ],
                        ),
                      ),
                    ),
                          ...
                          children: [
                            ...
                            const Padding(
                              ...
                              child: CircleAvatar(
                                ...
                                child: Icon(Icons.local_hospital, color: Color(0xFFFFBE0B)),
                              ),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                            ...
                            Padding(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () => context.read<ErcpCubit>().delete(item.id!),
                                tooltip: 'Delete',
                              ),
                            ),
                          ],
  ...
  void _showAddDialog(BuildContext context) {
    showDialog(
      ...
      builder: (dialogContext) => MultiBlocProvider(
        providers: [
          BlocProvider.value(value: context.read<ErcpCubit>()),
        ],
        child: _ErcpAddDialog(patientId: patient.id!),
      ),
    );
  }
}

class _ErcpAddDialog extends StatefulWidget {
  final int patientId;

  const _ErcpAddDialog({required this.patientId});

  @override
  State<_ErcpAddDialog> createState() => _ErcpAddDialogState();
}

class _ErcpAddDialogState extends State<_ErcpAddDialog> {
  ...
  void _submit() async {
    ...
    try {
      await context.read<ErcpCubit>().add(endoscopy);
      Navigator.of(context).pop();
    } catch (e) {
      print('Error adding ERCP: $e');
    }
  }
                              ...
                              child: const Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(width: 10),
                                    Text('Add ERCP', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 16)),
                                  ],
                                ),
                              ),
                            ),
