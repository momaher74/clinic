...
class PregnancyTestSection extends StatelessWidget {
  final Patient patient;
  const PregnancyTestSection({Key? key, required this.patient}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        context.read<PregnancyTestCubit>().loadForPatient(patient.id!, force: true);
      } catch (_) {}
    });

    return BlocBuilder<PregnancyTestCubit, PregnancyTestState>(
        ...
        if (state.list.isEmpty) {
          return Padding(
            ...
            child: Row(
              ...
              children: [
                const Text('Pregnancy Test', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: const [
                ...
                SizedBox(width: 8),
                Text('Add Beta HCG', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildCard(BuildContext context, PregnancyTest item) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Padding(
            ...
            child: CircleAvatar(radius: 22, backgroundColor: Color(0xFFEEF2FF), child: Icon(Icons.pregnant_woman, color: Color(0xFF3B82F6))),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    runSpacing: 6,
                    children: [
                      _infoChip('β-hCG', item.bhcg?.toString()),
                    ],
                  ),
                ],
              ),
          ...
          Padding(
            ...
            child: IconButton(onPressed: item.id == null ? null : () => context.read<PregnancyTestCubit>().delete(item.id!), icon: const Icon(Icons.delete_outline, color: Colors.grey)),
          ),
        ],
      ),
    );
  ...
  void _showAddDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (dialogContext) => BlocProvider.value(value: context.read<PregnancyTestCubit>(), child: AddPregnancyDialog(patientId: patient.id!)),
    );
  }
...
class AddPregnancyDialog extends StatefulWidget {
  final int patientId;
  const AddPregnancyDialog({Key? key, required this.patientId}) : super(key: key);

  @override
  State<AddPregnancyDialog> createState() => _AddPregnancyDialogState();
}
...
class _AddPregnancyDialogState extends State<AddPregnancyDialog> {
  ...
  final bhcg = TextEditingController();
  ...
  void _submit() async {
    ...
    try {
      await context.read<PregnancyTestCubit>().add(item);
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    const Text('Add Pregnancy Test', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                    const Spacer(),
                ...
                child: Form(
                  ...
                  child: Column(
                    children: [
                      ...
                      TextFormField(controller: bhcg, decoration: InputDecoration(labelText: 'β-hCG', border: const OutlineInputBorder(), filled: true, fillColor: Colors.grey.shade50), keyboardType: TextInputType.numberWithOptions(decimal: true), validator: (_) => null),
                      const SizedBox(height: 12),
                      SizedBox(
