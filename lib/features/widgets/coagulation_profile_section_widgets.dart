...
class CoagulationProfileSection extends StatelessWidget {
  final Patient patient;
  const CoagulationProfileSection({Key? key, required this.patient}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        context.read<CoagulationProfileCubit>().loadForPatient(patient.id!, force: true);
      } catch (_) {}
    });

    return BlocBuilder<CoagulationProfileCubit, CoagulationProfileState>(
        ...
        if (state.list.isEmpty) {
          return Padding(
            ...
            child: Row(
              ...
              children: [
                const Text('Coagulation Profile', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: const [
                ...
                SizedBox(width: 8),
                Text('Add Coagulation', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildCard(BuildContext context, CoagulationProfile item) {
    return Container(
        ...
        children: [
          ...
          const Padding(
            ...
            child: CircleAvatar(
              ...
              child: Icon(Icons.healing, color: Color(0xFF3B82F6)),
            ),
          ),

          const SizedBox(width: 12),

          // Text and chips section
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    runSpacing: 6,
                    children: [
                      _infoChip('PT', item.pt?.toString()),
                      _infoChip('PTT', item.ptt?.toString()),
                      _infoChip('PC', item.pc?.toString()),
                      _infoChip('INR', item.inr?.toString()),
                    ],
                  ),
                ],
              ),
          ...
          Padding(
            ...
            child: IconButton(
              onPressed: item.id == null ? null : () => context.read<CoagulationProfileCubit>().delete(item.id!),
  ...
  void _showAddDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (dialogContext) => BlocProvider.value(value: context.read<CoagulationProfileCubit>(), child: AddCoagulationDialog(patientId: patient.id!)),
    );
  }
...
class AddCoagulationDialog extends StatefulWidget {
  final int patientId;
  const AddCoagulationDialog({Key? key, required this.patientId}) : super(key: key);
...
class _AddCoagulationDialogState extends State<AddCoagulationDialog> {
  ...
  void _submit() async {
    ...
    final item = CoagulationProfile(
      patientId: widget.patientId,
      date: _date!.toIso8601String(),
      pt: _parseNum(ptCtrl),
      ptt: _parseNum(pttCtrl),
      pc: _parseNum(pcCtrl),
      inr: _parseNum(inrCtrl),
      createdAt: DateTime.now().toIso8601String(),
    );

    try {
      await context.read<CoagulationProfileCubit>().add(item);
                ...
                child: Form(
                  ...
                  child: Column(
                    children: [
                      ...
                      _NumberField(controller: ptCtrl, label: 'PT'),
                      _NumberField(controller: pttCtrl, label: 'PTT'),
                      _NumberField(controller: pcCtrl, label: 'PC'),
                      _NumberField(controller: inrCtrl, label: 'INR'),
                      const SizedBox(height: 12),
                      SizedBox(
